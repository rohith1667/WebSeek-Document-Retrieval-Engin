...
class TextVectorizer:
    ...
    def query(self, query, k=10):
        ...
        # # Find the closest text using Word2Vec
        # # query_vector = [self.word2vec_model.wv.word_vec(item) for item in query.split() if item in self.word2vec_model.wv.key_to_index]
        # # print(query_vector)
        # # Find the closest text using Word2Vec
        # # # query_vector = self.word2vec_model.wv[query.split()]
        # # cosine_sim = self.word2vec_model.wv.similarity(
        #     query_vector, self.word2vec_model.wv.vectors_for_all)
        # # # The last one is the most similar
        # # index_word2vec = cosine_sim.argsort()[0][-1]
